...
import { Lightbulb, CalendarDays, Pencil, Trash2, Plus, RotateCcw, X, Check, Archive } from 'lucide-react';
...
function ConceptForm({ initial, color, onSave, onCancel }) {
  const [f, setF] = useState(() => ({
    ...
    desc: initial.desc || '', budget: initial.budget || '', tag: initial.tag || 'new',
  ...
  const save = () => {
    ...
    onSave({ name: f.name.trim(), offer: f.offer.trim(), timing: f.timing.trim(), desc: f.desc.trim(), budget: f.budget.trim(), tag: f.tag, span: [s0, s1], moment: momentLabel([s0, s1]) });
  };
  ...
  return (
    <div className="bg-[#0d0d0d] border rounded-md p-3 flex flex-col gap-2" style={{ borderLeft: `3px solid ${color}` }}>
      ...
      <div className="flex gap-2">
        <input className={inp} placeholder="Offre (Event · BC…)" value={f.offer} onChange={e => set('offer', e.target.value)} />
        <select className={`${inp} w-28 shrink-0`} value={f.tag} onChange={e => set('tag', e.target.value)}>
          <option value="new">NET NEW</option><option value="piste">PISTE BP</option><option value="recond">RECOND</option>
        </select>
      </div>
...
function GanttRow({ label, marker, span, color, filled }) {
  const [s, e] = span || [0, 0];
  return (
    <div className="grid items-center min-h-[30px] border-t border-[rgba(250,250,247,.05)]" style={{ gridTemplateColumns: '180px repeat(15, minmax(26px, 1fr))' }}>
      <div className="text-[11px] text-[#ccc] pr-2 truncate flex items-center gap-1"><span style={{ color }}>{marker}</span>{label}</div>
      <div className="h-[26px] flex items-center" style={{ gridColumn: `${s + 2} / ${e + 3}` }}>
        <div className="h-[14px] w-full rounded-full" style={filled ? { backgroundColor: color } : { border: `1px solid ${color}` }} />
      </div>
    </div>
  );
}
...
function EventForm({ initial, accent, onSave, onCancel }) {
  ...
  return (
    <div className="grid grid-cols-[120px_1fr] gap-3 py-3 border-t border-[#222]">
      <input className={`${inp} w-full`} placeholder="Date (ex. Juin 2026)" value={f.date} onChange={e => set('date', e.target.value)} autoFocus />
...
export default function DCFarming() {
  ...
  const [editConcept, setEditConcept] = useState(null); // id en édition | 'NEW'
  const [editEvent, setEditEvent] = useState(null);      // id en édition | 'NEW'
  const [trashConcepts, setTrashConcepts] = useState(false);
  const [trashEvents, setTrashEvents] = useState(false);

  useEffect(() => {
    let alive = true;
    apiFetch(`/api/data/farming?dc=${DC}`)
  ...
  // Persiste l'état complet d'un client (optimiste)
  const persist = useCallback((clientName, data) => {
    setClientsData(prev => ({ ...prev, [clientName]: data }));
    apiFetch('/api/data/farming', { method: 'PUT', body: JSON.stringify({ dc: DC, client: clientName, data }) })
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        {editConcept === 'NEW' && (
          <div className="mb-3 max-w-md"><ConceptForm initial={{}} color={accent} onSave={addConcept} onCancel={() => setEditConcept(null)} /></div>
        )}
      ...
      <div>
        ...
        <div className="bg-[#161616] border border-[#2a2a2a] rounded-lg p-4 overflow-x-auto">
          <div style={{ minWidth: 860 }}>
            ...
            {events.map(ev => <GanttRow key={ev.id} label={ev.title} marker="◇" span={ev.span} color={accent} filled={false} />)}
            ...
            {concepts.map(c => <GanttRow key={c.id} label={c.name} marker="●" span={c.span} color={accent} filled={true} />)}
          </div>
        </div>

        {/* Liste éditable des événements */}
        <div className="bg-[#161616] border border-[#2a2a2a] rounded-lg px-4 py-1 mt-3">
          {editEvent === 'NEW' && <EventForm initial={{}} accent={accent} onSave={addEvent} onCancel={() => setEditEvent(null)} />}
          {events.map(ev => editEvent === ev.id
            ? <EventForm key={ev.id} initial={ev} accent={accent} onSave={f => saveEventEdit(ev.id, f)} onCancel={() => setEditEvent(null)} />
            : (
              <div key={ev.id} className="group grid grid-cols-[120px_1fr] gap-3 py-3 border-t border-[#222] first:border-t-0">
