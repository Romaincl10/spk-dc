...
export default function DCSynthese({ portfolio, color, viewMode = 'signe', fyStartYear }) {
  ...
  const [trackMode, setTrackMode] = useState('cumul'); // 'cumul' | 'agence' | 'medias'
  const detailRef = useRef(null);
  const clientTracking = portfolio.clientTracking || [];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Suivi regroupé des clients : CA réalisé vs Objectif (Cumul / Agence / Médias) ── */}
      {clientTracking.length > 0 && (
        <div className="bg-[#161616] border border-[#2a2a2a] rounded-xl p-5">
            ...
            <div className="flex gap-1 bg-[#111] rounded-lg p-0.5">
              {[['cumul', 'Cumul'], ['agence', 'Agence'], ['medias', 'Médias']].map(([id, label]) => (
                <button key={id} onClick={() => setTrackMode(id)}
                  className={`px-3 py-1.5 text-xs font-bold rounded-md transition-colors ${trackMode === id ? 'bg-[#e63946] text-white' : 'text-[#888] hover:text-white'}`}>{label}</button>
              ))}
            </div>
          </div>
          {(() => {
            const rows = clientTracking.map(c => {
              const ca = trackMode === 'agence' ? c.caAgence : trackMode === 'medias' ? c.caMedia : c.caAgence + c.caMedia;
              const obj = trackMode === 'agence' ? c.objAgence : trackMode === 'medias' ? c.objMedia : c.objAgence + c.objMedia;
              return { client: c.client, ca, obj, pct: obj > 0 ? Math.round(ca / obj * 100) : null };
            }).filter(r => r.ca > 0 || r.obj > 0).sort((a, b) => b.ca - a.ca);
            const totCA = rows.reduce((s, r) => s + r.ca, 0), totObj = rows.reduce((s, r) => s + r.obj, 0);
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    <tr className="border-t-2 border-[#e63946]/30 bg-[#e63946]/5 font-bold">
                      <td className="py-2 px-2 text-white">TOTAL {trackMode === 'cumul' ? '(Agence + Médias)' : trackMode === 'agence' ? '(Agence)' : '(Médias)'}</td>
